...
class FakeQueryJobSpec extends fixture.FunSpec with MustMatchers with DBFixture {
  ...
  describe("getResult") {
    it("works with non-partitioned table") { conn =>
      ...
      withFakeTable(conn, tableDefinition) { fakeTable =>
        fakeTable.fakeBigQuery.queryHelper.execute(
        ...
        val result = new FakeQueryJob(fakeTable.fakeBigQuery, queryJobConfiguration).fetchResult()
    ...
    it("works with partitioned table") { conn =>
      val schema = Schema.of(
        Field.of("text", LegacySQLTypeName.STRING)
      )
    ...
    it("works with all field types") { conn =>
      val schema = Schema.of(
      ...
      withFakeTable(conn, tableDefinition) { fakeTable =>
        fakeTable.fakeBigQuery.queryHelper.execute(
          """
            ...
            | VALUES ('4142', 'text', 1, 2.0, 3, TRUE, '2019-01-01 19:01:01', '2019-01-02', '20:19:01', '2018-12-31 23:59:59');
          """.stripMargin
        )
        val queryJobConfiguration = QueryJobConfiguration.newBuilder("SELECT * FROM bigqueryfake.test").build
        val result = new FakeQueryJob(fakeTable.fakeBigQuery, queryJobConfiguration).fetchResult()
