...
class FakeTableSpec extends fixture.FunSpec with MustMatchers with DBFixture {
  def withFakeTable(conn: Connection)(test: FakeTable => Any) {
  ...
  describe("create") {
    it("creates a non-partitioned table") { conn =>
      withFakeTable(conn) { fakeTable =>
        ...
        fakeTable.fakeBigQuery.queryHelper.
    ...
    it("creates a partitioned table") { conn =>
      withFakeTable(conn) { fakeTable =>
        val schema = Schema.of(Field.of("text", LegacySQLTypeName.STRING))
        ...
        fakeTable.create(tableDefinition).getTableId.getTable mustBe "test"
        fakeTable.fakeBigQuery.queryHelper.
          list("SELECT column_name FROM INFORMATION_SCHEMA.columns WHERE table_schema = 'bigqueryfake' and table_name = 'test';").toSet mustBe Set("text", "_PARTITIONTIME")
      }
    }
  }

  describe("get") {
    it("returns a non-partitioned table") { conn =>
      withFakeTable(conn) { fakeTable =>
        fakeTable.fakeBigQuery.queryHelper.execute("CREATE TABLE bigqueryfake.test (name VARCHAR(65536));")
    ...
    it("returns a partitioned table") { conn =>
      withFakeTable(conn) { fakeTable =>
        fakeTable.fakeBigQuery.queryHelper.execute("CREATE TABLE bigqueryfake.test (name VARCHAR(65536), _PARTITIONTIME timestamp);")
        val tableDefinition = fakeTable.get.get.getDefinition[StandardTableDefinition]
        tableDefinition.getSchema.getFields.iterator().asScala.map(_.getName).toSet mustBe Set("name")
  ...
  describe("update") {
    it("do nothing and returns a table") { conn =>
      withFakeTable(conn) { fakeTable =>
        fakeTable.fakeBigQuery.queryHelper.execute("CREATE TABLE bigqueryfake.test (name VARCHAR(65536), _PARTITIONTIME timestamp);")
  ...
  describe("list") {
    it("returns all tables") { conn =>
      withFakeTable(conn) { fakeTable =>
        val fakeBigQuery = fakeTable.fakeBigQuery
